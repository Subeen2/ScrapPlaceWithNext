...
function Detail(props: Props) {
  ...
  useEffect(() => {
    ...
    const iwContent = '<div style="padding:5px;">' + props.place.placename + '<br><a href="https://map.kakao.com/link/to/' + props.place.placename + ',33.450701,126.570667" style="color:blue" target="_blank">길찾기</a></div>', // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
  ...
  return <div className='detail-box'>
    <div>
      ...
      <div className='info-box'>
        <div>주소 :</div>
        <span>{props.place.address}</span><br />
        <div>영업 시간 :</div>
        <span>{props.place.openingHours}</span><br />
        <div>브레이크 타임 :</div>
        <span>{props.place.breakTime}</span><br />
        <div>휴무일 :</div>
        <span>{props.place.offDay}</span><br />
        <div>연락처 :</div>
        <span>{props.place.contact}</span>
      </div>
      <div id='map'></div>
      <div className='content'>마우스 왼쪽 클릭 → 원하는 장소에 옮겨 재클릭 → 오른쪽 마우스 클릭 : 장소 사이의 거리, 도보/자전거 이동시 소요 시간을 보실 수 있습니다 . !</div>
    </div>
    {/* <div>
      <span>근처</span>
